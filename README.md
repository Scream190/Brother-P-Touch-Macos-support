...
from CUPS/PPD configuration issues.
...
# 2. Find the paired device name:
python3 tools/list_bt_serial_ports.py

# 3. Send real test prints, roughly in this order:
python3 tools/test_print.py --media 12mm --pattern solid   --device NAME  # prints at all, full width?
python3 tools/test_print.py --media 12mm --pattern ruler    --device NAME  # tape width detected & centered correctly?
python3 tools/test_print.py --media 12mm --pattern diagonal --device NAME  # any bit/byte-order bugs? (line should be straight)
python3 tools/test_print.py --media 12mm --pattern checkerboard --device NAME
python3 tools/test_print.py --media 12mm --pattern border   --device NAME  # edges clipped?
python3 tools/test_print.py --media 12mm --pattern stripes  --device NAME
```

Swap `--media` to whatever cassette you actually have loaded. Run
...
tools/test_print.py       Standalone hardware test tool (bypasses CUPS entirely)
